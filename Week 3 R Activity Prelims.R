...
getwd()
setwd("C:/Users/aalshi/Desktop/Week3")
...
weight <- women_new$weight
height <- women_new$height
...
women_subset <- subset(women_new,height>65)
...
ggplot(women_new, aes(x=height, y=weight)) + 
  geom_point()

#Optional 'Just for fun' TASK: Change the arguments for the geom point and see what happens to the plot
ggplot(women_new, aes(x=height, y=weight)) + 
  geom_point(shape = 21, colour = "yellow", fill = "blue", size = 10, stroke = 10)
